...
public:
  ...
  void init(VertexId_CUDA v_size_, VertexId_CUDA e_size_,
    ...
    column_offset =
        (VertexId_CUDA *)cudaMallocGPU((v_size_ + 1) * sizeof(VertexId_CUDA));
    row_indices =
        (VertexId_CUDA *)cudaMallocGPU((e_size_) * sizeof(VertexId_CUDA));
    if (require_mirror_) {
      require_mirror = require_mirror_;
      mirror_size = mirror_size_;
      mirror_index = (VertexId_CUDA *)cudaMallocGPU((mirror_size_) *
                                                    sizeof(VertexId_CUDA));
    }
  }
  void load_from_host(VertexId_CUDA *h_column_offset,
                      VertexId_CUDA *h_row_indices,
                      VertexId_CUDA *h_mirror_index) {
    // printf("%d %d %d \n",v_size,e_size,mirror_size);
    move_bytes_in(column_offset, h_column_offset,
                  (v_size + 1) * sizeof(VertexId_CUDA));
    move_bytes_in(row_indices, h_row_indices, (e_size) * sizeof(VertexId_CUDA));
    move_bytes_in(mirror_index, h_mirror_index,
                  (mirror_size) * sizeof(VertexId_CUDA));
  }
  void load_from_host(VertexId_CUDA *h_column_offset,
                      VertexId_CUDA *h_row_indices) {
    move_bytes_in(column_offset, h_column_offset,
...
public:
  ...
  void Scatter_Src_Mirror_to_Msg(float *message,
                                 float *src_mirror_feature, // data
                                 VertexId_CUDA *row_indices,
                                 VertexId_CUDA *column_offset,
                                 VertexId_CUDA *mirror_index,
  ...
  void Gather_Msg_To_Src_Mirror(float *src_mirror_feature,
                                float *message, // data
                                VertexId_CUDA *row_indices,
                                VertexId_CUDA *column_offset,
                                VertexId_CUDA *mirror_index,
  ...
  void Edge_Softmax_Backward_Block(float *msg_input_grad,
                                   float *msg_output_grad, // data
                                   float *msg_cached,
                                   VertexId_CUDA *row_indices,
                                   VertexId_CUDA *column_offset,
                                   VertexId_CUDA batch_size,
  ...
  //                           VertexId_CUDA *src, VertexId_CUDA *dst,
  //                           VertexId_CUDA *src_index, VertexId_CUDA
  //                           *dst_index, float *weight_buffer, int dst_offset,
  //                           int dst_offset_end, int feature_size, int
  //                           edge_size, int out_put_buffer_size, bool sync =
  //                           true);
