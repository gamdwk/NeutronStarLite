    ...
    for (int i = 0; i < graph->gnnctx->layer_size.size() - 1; i++) {
      ...
      if (i == 0) {
        ...
        for (VertexId val = graph->partition_offset[graph->partition_id];
          ...
          X[i][val - graph->partition_offset[graph->partition_id]][10] =
              (float)(val);
        //         LOG_INFO("X_i[0][10](%f, %f)",X[i][testid][0]);
        // test mirror
        NtsVar mirror = ctx->runGraphOp<nts::op::DistGetDepNbrOp>(
            partitioned_graph, active, X[i]);
        NtsVar x_trans = X[i].cuda();
        NtsVar mirror_gpu = ctx->runGraphOp<nts::op::DistGPUGetDepNbrOp>(
            partitioned_graph, active, x_trans);
        // NtsVar
        // test_mirror_forward=torch::cat({mirror.slice(1,10,11,1),mirror_gpu.cpu().slice(1,10,11,1)},1);
        // NtsVar
        // mirror.slice(1,10,11,1).eq(mirror_gpu.cpu().slice(1,10,11,1)).sum(0);
        partitioned_graph->SyncAndLog(
            "sync test DistGPUGetDepNbrOp forward: 1 is passed");
        std::cout << mirror.slice(1, 10, 11, 1)
                         .eq(mirror_gpu.cpu().slice(1, 10, 11, 1))
                         .sum(0)
                         .eq(mirror.size(0))
                  << std::endl;
        ...
        partitioned_graph->SyncAndLog(
            "sync test DistGPUGetDepNbrOp backward: 1 is passed");
        std::cout << x_back.slice(1, 10, 11, 1)
                         .eq(x_back_cuda.cpu().slice(1, 10, 11, 1))
                         .sum(0)
                         .eq(x_back.size(0))
                  << std::endl;
      }
  ...
  void test_scatter_src() {
    ...
    for (int i = 0; i < graph->gnnctx->layer_size.size() - 1; i++) {
      ...
      if (i == 0) {
        ...
        NtsVar edge_src = ctx->runGraphOp<nts::op::DistScatterSrc>(
            partitioned_graph, active, mirror);
        NtsVar edge_src_gpu = ctx->runGraphOp<nts::op::DistGPUScatterSrc>(
            partitioned_graph, active, mirror_gpu);
        partitioned_graph->SyncAndLog(
            "sync test DistGPUScatterSrc forward: 1 is passed");
        std::cout << edge_src.slice(1, 10, 11, 1)
                         .eq(edge_src_gpu.cpu().slice(1, 10, 11, 1))
                         .sum(0)
                         .eq(edge_src.size(0))
                  << std::endl;
        ...
        partitioned_graph->SyncAndLog(
            "sync test DistGPUScatterSrc backward: 1 is passed");
        std::cout << x_back.slice(1, 10, 11, 1)
                         .eq(x_back_cuda.cpu().slice(1, 10, 11, 1))
                         .sum(0)
                         .eq(x_back.size(0))
      ...
      if (i == 0) {
        ...
        for (VertexId val = graph->partition_offset[graph->partition_id];
          ...
          for (VertexId j = 0; j < X[i].size(1); j++)
            X[i][val - graph->partition_offset[graph->partition_id]][j] =
                (float)(val);
        ...
        NtsVar edge_dst = ctx->runGraphOp<nts::op::DistScatterDst>(
            partitioned_graph, active, X[i]);
        NtsVar edge_dst_gpu = ctx->runGraphOp<nts::op::DistGPUScatterDst>(
            partitioned_graph, active, X_i_gpu);
        partitioned_graph->SyncAndLog(
            "sync test DistGPUScatterDst forward: 1 is passed");
        std::cout << edge_dst.eq(edge_dst_gpu.cpu())
                         .sum(0)
                         .sum(0)
                         .eq(edge_dst.size(0) * edge_dst.size(1))
                  << std::endl;
        ...
        partitioned_graph->SyncAndLog(
            "sync test DistGPUScatterDst backward: 1 is passed");
        std::cout << x_back.eq(x_back_cuda.cpu())
                         .sum(0)
                         .sum(0)
                         .eq(x_back.size(0) * x_back.size(1))
                  << std::endl;
      }
    }
  }
  void test_gather_dst() {
    ...
    for (int i = 0; i < graph->gnnctx->layer_size.size() - 1; i++) {
      ...
      if (i == 0) {
        ...
        NtsVar dst_y = ctx->runGraphOp<nts::op::DistAggregateDst>(
            partitioned_graph, active, edge_dst);
        NtsVar dst_y_gpu = ctx->runGraphOp<nts::op::DistGPUAggregateDst>(
            partitioned_graph, active, edge_dst_gpu);

        partitioned_graph->SyncAndLog(
            "sync test DistGPUAggregateDst forward: 1 is passed");
        std::cout << dst_y.eq(dst_y_gpu.cpu())
                         .sum(0)
                         .sum(0)
                         .eq(dst_y.size(0) * dst_y.size(1))
                  << std::endl;
        ...
        partitioned_graph->SyncAndLog(
            "sync test DistGPUAggregateDst backward: 1 is passed");
        std::cout << x_back.eq(x_back_cuda.cpu())
                         .sum(0)
                         .sum(0)
                         .eq(x_back.size(0) * x_back.size(1))
    ...
    for (int i = 0; i < graph->gnnctx->layer_size.size() - 1; i++) {
      ...
      if (i == 0) {
        ...
        NtsVar a = ctx->runGraphOp<nts::op::DistEdgeSoftMax>(partitioned_graph,
                                                             active, m);
        NtsVar a_gpu = ctx->runGraphOp<nts::op::DistGPUEdgeSoftMax>(
            partitioned_graph, active, m_gpu);
        NtsVar test_mirror_forward = torch::cat({a, a_gpu.cpu()}, 1);
        //   std::cout<<test_mirror_forward.slice(0,1119,2200,1)<<std::endl;
        partitioned_graph->SyncAndLog(
            "sync test DistGPUEdgeSoftMax forward: 1 is passed");
        std::cout
            << torch::abs(a - a_gpu.cpu()).le(0.0000001).sum(0).eq(a.size(0))
            << std::endl;
        ...
        partitioned_graph->SyncAndLog(
            "sync test DistGPUEdgeSoftMax backward: 1 is passed");
        // std::cout<<(x_back-x_back_cuda.cpu()).slice(0,0,6,1)<<std::endl;
        std::cout << torch::abs(x_back - x_back_cuda.cpu())
                         .le(0.0000001)
                         .sum(0)
                         .eq(x_back.size(0))
                  << std::endl;
