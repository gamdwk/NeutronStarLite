...
public:
  ...
  void init_graph() {
    ...
    //    // generate the representation for subgraph corresponding to the way
    //    we
    ...
    //    // pre-process the data that will be used while doing forward and
    //    backward
    ...
    for (int i = 0; i < graph->gnnctx->layer_size.size() - 1; i++) {
      ...
      if (i == 0) {
        ...
        for (VertexId val = graph->partition_offset[graph->partition_id];
          ...
          X[i][val - graph->partition_offset[graph->partition_id]][0] =
              (float)1;
        ...
        //           std::cout<<partitioned_graph->row_indices[i]<<"
        //           "<<edge_src[i][0]<<std::endl;
        //        }
        ...
        //         for(int i=0;i<partitioned_graph->owned_vertices;i++){
        //               std::cout<<graph->out_degree_for_backward[i+graph->gnnctx->p_v_s]<<"
        //               "<<X_s[i][0]<<std::endl;
        //         }

        // test DistScatterDst
        //         NtsVar edge_dst=
        //         ctx->runGraphOp<nts::op::DistScatterDst>(partitioned_graph,active,X[i]);
        //         for(int i=0;i<partitioned_graph->owned_vertices;i++){
        //            for(int j=partitioned_graph->column_offset[i];
        //                    j<partitioned_graph->column_offset[i+1];j++){
        //               std::cout<<i+graph->gnnctx->p_v_s<<"
        //               "<<edge_dst[j][0]<<std::endl;
        //            }
        //         }
        ...
        //             if((partitioned_graph->MirrorIndex[i+1]-partitioned_graph->MirrorIndex[i])>0)
        //             std::cout<<i<<"
        //             "<<Y_i[partitioned_graph->MirrorIndex[i]][0]<<std::endl;
        //         }
        ...
        //        if(graph->partition_id==0){
        //            int i=1020;
        //            std::cout<<i+graph->partition_offset[graph->partition_id]<<"t
        //            "<<X_i[i][0]<<std::endl;
        //       }
        //       }

        // test Aggregate
        NtsVar Y = ctx->runGraphOp<nts::op::DistAggregateDst>(partitioned_graph,
                                                              active, edge_src);
      ...
      //       NtsVar Y_i=
      //       ctx->runGraphOp<nts::op::ForwardCPUfuseOp>(partitioned_graph,active,X[i]);
