...
MessageBuffer::MessageBuffer()
    : capacity(0), count(0), data(nullptr), pinned(false) {}
...
/**
 * @brief
 * get corresponding msg data, according to the layout mentioned in
 * Messagebuffer, we should add another unit offset while trying to get data
 ...
 * @param lsbl local send buffer limit. count for local_send_buffer should less
 * than lsbl
 */
void NtsGraphCommunicator::init(VertexId *partition_offset_,
                                VertexId owned_vertices_, VertexId partitions_,
                                VertexId sockets_, VertexId threads_,
...
/**
 * @brief
 * init communicator for this layer. i.e. allocating send_queue, send_buffer
 * etc.
 ...
 */
void NtsGraphCommunicator::init_layer_all(VertexId feature_size_, CommType et,
                                          DeviceLocation dl) {
...
void NtsGraphCommunicator::init_message_buffer_master_to_mirror() {
  for (int i = 0; i < partitions; i++) {
    for (int s_i = 0; s_i < sockets; s_i++) {
      // since we will also send and recv message locally, so we don't use
      // find-grained buffer which might introduce more code and increase
      // complexity for every recv buffer, resize to feature_size * (vertex_num
      // for this parittion) * sockets
      recv_buffer[i][s_i]->resize(
          size_of_msg(feature_size) *
          (partition_offset[i + 1] - partition_offset[i]) * sockets);
      // for every send buffer, resize to feature_size * owned_vertex_num *
      // sockets
...
void NtsGraphCommunicator::init_message_buffer_mirror_to_master() {
  ...
  for (int i = 0; i < partitions; i++) {
    for (int s_i = 0; s_i < sockets; s_i++) {
      // for every recv buffer, we are the master, and thus, using
      // owned_vertex_num
      recv_buffer[i][s_i]->resize(size_of_msg(feature_size) * owned_vertices *
                                  sockets);
      // for sending, buffer size should be the owned_vertex_num for that
      // partition
 ...
 * @param partition_id_ partition id
 * @param flush_local_buffer whether we need to flush local buffer to send
 * buffer
 */
 ...
 */
void NtsGraphCommunicator::achieve_local_message(
    VertexId current_send_partition_id_) {
 ...
 * @param step the step which specifies the partitionID. think about the steps
 * in ring scheduling
 * @return MessageBuffer** message buffer for all sockets from one partition
 */
MessageBuffer **NtsGraphCommunicator::recv_one_partition(int &workerId,
                                                         int step) {
  ...
  // the equivalent of c++ style loop is the combination of atmoic load and
  // _mm_pause wait till this partiiton is avaliable the reason we use spinlock
  // is because the cv is heavy-weight
  while (true) {
    recv_queue_mutex.lock();
 ...
 */
void NtsGraphCommunicator::emit_buffer(VertexId vtx, ValueType *buffer,
                                       int f_size) {
...
/**
 * @brief
 * place the data directly on send buffer based on vertexID instead of local
 * send buffer
 * @param vtx source vertex
 * @param buffer vertex feature data buffer
 ...
 * @param f_size feature size
 */
void NtsGraphCommunicator::emit_buffer_lock_free(VertexId vtx,
                                                 ValueType *buffer,
...
//      printf("MI-MA SEND 8GB\n");
      ...
      MPI_Send(send_buffer[i][s_i]->data,
               size_of_msg(feature_size) * send_buffer[i][s_i]->count, MPI_CHAR,
    ...
    recv_threads.emplace_back(
        [&](int i) {
          for (int s_i = 0; s_i < sockets; s_i++) {
            ...
            MPI_Recv(recv_msg, recv_buffer[i][s_i]->count, MPI_FLOAT, i,
...
void NtsGraphCommunicator::recv_master_to_mirror() {
  for (int step = 1; step < partitions; step++) {
    ...
    for (int s_i = 0; s_i < sockets; s_i++) {
      ...
      MPI_Get_count(&recv_status, MPI_CHAR, &recv_buffer[i][s_i]->count);
      MPI_Recv(recv_buffer[i][s_i]->data, recv_buffer[i][s_i]->count, MPI_CHAR,
               i, PassMessage, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      recv_buffer[i][s_i]->count /= size_of_msg(feature_size);

      //    for messages less than 8GB
      //      float* recv_msg=(float*)recv_buffer[i][s_i]->data;
      //      MPI_Get_count(&recv_status, MPI_FLOAT,
      //      &recv_buffer[i][s_i]->count); MPI_Recv(recv_msg,
 ...
 * And push partition ID into recv_queue to indicate the corresponding partition
 * is ready
 */
...
//            printf("MI-MA RECV 8GB\n");
  ...
  // last partition is local partition. we will retrive messages from send
  // buffer directly
...
void NtsGraphCommunicator::flush_local_send_buffer_buffer(int t_i, int f_size) {
  ...
  int pos = __sync_fetch_and_add(&send_buffer[current_send_part_id][s_i]->count,
