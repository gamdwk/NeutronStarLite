...
public:
  ...
  NtsVar forward(NtsVar &f_input) { // input edge  output vertex
    ...
    NtsVar f_output = graph_->Nts->NewKeyTensor(
        {partitioned_graph_->owned_mirrors, feature_size},
    ...
    // LOG_INFO("owned_mirrors (%d)",partitioned_graph_->owned_mirrors);
    graph_->get_from_dep_neighbor_mutisockets<
        ...
        [&](VertexId dst, ValueType *recv_buffer, VertexId recv_id) {
          memcpy(f_output_buffer +
                     partitioned_graph_->MirrorIndex[dst] * feature_size,
                 recv_buffer, sizeof(ValueType) * feature_size);
          return 0;
        },
  ...
  NtsVar backward(NtsVar &f_output_grad) { // input vtx grad; output edge grad
    ...
    NtsVar f_input_grad = graph_->Nts->NewLeafTensor(
        {graph_->gnnctx->l_v_num, feature_size}, torch::DeviceType::CPU);
    ...
    graph_->process_edges_backward_decoupled_multisockets<
        ...
        [&](VertexId src, VertexAdjList<Empty> outgoing_adj, VertexId thread_id,
          ...
          ValueType *local_output_buffer = // new ValueType[2048];
              f_output_grad_buffer +
              partitioned_graph_->MirrorIndex[src] * feature_size;
          if (graph_->rtminfo->lock_free) {
        ...
        [&](VertexId src, ValueType *msg) {
          nts_acc(f_input_grad_buffer +
                      (src - graph_->gnnctx->p_v_s) * feature_size,
                  msg, feature_size);
          return 1;
        },
...
public:
  ...
  NtsVar forward(NtsVar &f_input) { // input edge  output vertex
    ...
    // LOG_INFO("owned_mirrors (%d)",partitioned_graph_->owned_mirrors);
    NtsVar f_output = graph_->Nts->NewKeyTensor(
        {partitioned_graph_->owned_edges, feature_size},
        torch::DeviceType::CPU);
    ValueType *f_input_buffer =
        graph_->Nts->getWritableBuffer(f_input, torch::DeviceType::CPU);
    ValueType *f_output_buffer =
    ...
    partitioned_graph_->DistSchedulingMaster(
        [&](VertexId dst, PartitionedGraph *pg) {
          ...
          for (int eid = pg->column_offset[dst_trans];
            ...
            nts_copy(f_output_buffer, eid, f_input_buffer, src_pos,
                     feature_size, 1);
          }
        });
    return f_output;
        ...
        [&](VertexId dst, PartitionedGraph *pg) {
          ...
          for (int eid = pg->column_offset[dst_trans];
            ...
            nts_acc(f_input_grad_buffer + src_pos * feature_size,
                    f_output_grad_buffer + eid * feature_size, feature_size);
          }
        });
  ...
  NtsVar forward(NtsVar &f_input) { // input edge  output vertex
    ...
    partitioned_graph_->DistSchedulingMaster(
        [&](VertexId dst, PartitionedGraph *pg) {
          ...
          for (int eid = pg->column_offset[dst_trans];
               eid < pg->column_offset[dst_trans + 1]; eid++) {
            nts_copy(f_output_buffer, eid, f_input_buffer, dst_trans,
                     feature_size, 1);
          }
        });
    return f_output;
...
public:
  ...
  NtsVar forward(NtsVar &f_input) { // input edge  output vertex
    ...
    // LOG_INFO("owned_mirrors (%d)",partitioned_graph_->owned_mirrors);
    NtsVar f_output = graph_->Nts->NewKeyTensor(
        {partitioned_graph_->owned_vertices, feature_size},
        torch::DeviceType::CPU);
    ValueType *f_input_buffer =
        graph_->Nts->getWritableBuffer(f_input, torch::DeviceType::CPU);
    ValueType *f_output_buffer =
    ...
    partitioned_graph_->DistSchedulingMaster(
        [&](VertexId dst, PartitionedGraph *pg) {
          ...
          for (int eid = pg->column_offset[dst_trans];
            ...
            //                                feature_size);
            nts_copy(f_input_grad_buffer, eid, f_output_grad_buffer, dst_trans,
                     feature_size, 1);
          }
        });
...
public:
  ...
  NtsVar backward(NtsVar &f_output_grad) { // input vtx grad; output edge grad
    ...
    partitioned_graph_->DistSchedulingMaster(
        [&](VertexId dst, PartitionedGraph *pg) {
          VertexId dst_trans = dst - graph_->gnnctx->p_v_s;
          nts_assign(f_input_grad_buffer,
                     f_output_grad_buffer + feature_size * dst_trans,
  ...
  NtsVar forward(NtsVar &f_input_) { // input i_msg  output o_msg
    ...
    partitioned_graph_->DistSchedulingMaster(
        [&](VertexId dst, PartitionedGraph *pg) {
          ...
          // NtsVar d= f_input_.slice(0, eid_start, eid_end,
          // 1)*torch::exp(f_input_.slice(0, eid_start, eid_end, 1)).sum(0);///
          // torch::exp(f_input_.slice(0, eid_start, eid_end, 1)).sum(0);
          ValueType *d_buffer =
              graph_->Nts->getWritableBuffer(d, torch::DeviceType::CPU);
          nts_copy(f_output_buffer, eid_start, d_buffer, 0, feature_size,
                   (eid_end - eid_start));
        });
  ...
  NtsVar backward(NtsVar &f_output_grad) { // input vtx grad; output edge grad
    ...
    partitioned_graph_->DistSchedulingMaster(
        [&](VertexId dst, PartitionedGraph *pg) {
          ...
          nts_copy(f_input_grad_buffer, eid_start, d_o_buffer, 0, feature_size,
                   (eid_end - eid_start));
        });
    return f_input_grad;
  }
...
public:
  ...
  DistAggregateDstFuseWeight(PartitionedGraph *partitioned_graph,
                             VertexSubset *active)
      : ntsGraphOp(partitioned_graph, active) {
    // subgraphs = partitioned_graph->graph_chunks;
  }
  ...
  NtsVar forward(NtsVar &f_input,
                 NtsVar &e_weight) { // input edge  output vertex
    int feature_size = f_input.size(1);
    // LOG_INFO("owned_mirrors (%d)",partitioned_graph_->owned_mirrors);
    NtsVar f_output = graph_->Nts->NewKeyTensor(
        {partitioned_graph_->owned_vertices, feature_size},
    ...
    partitioned_graph_->DistSchedulingMaster([&](VertexId dst,
                                                 PartitionedGraph *pg) {
      VertexId dst_trans = dst - graph_->gnnctx->p_v_s;
  ...
  NtsVar backward(NtsVar &f_output_grad) { // input vtx grad; output edge grad
    ...
    NtsVar f_input_grad = graph_->Nts->NewLeafTensor(
        ...
        torch::DeviceType::CPU);
    e_weight_grad =
        graph_->Nts->NewLeafTensor(e_weight_cache, torch::DeviceType::CPU);
    ValueType *f_input_grad_buffer =
        graph_->Nts->getWritableBuffer(f_input_grad, torch::DeviceType::CPU);
    ValueType *e_weight_buffer =
