...
public:
  ...
  NtsVar forward(NtsVar &f_input) { // input edge  output vertex
    ...
    // LOG_INFO("owned_mirrors (%d)",partitioned_graph_->owned_mirrors);
    cuda_stream->Scatter_Src_Mirror_to_Msg(
        f_output_buffer, f_input_buffer, device_csc->row_indices,
  ...
  NtsVar backward(NtsVar &f_output_grad) { // input vtx grad; output edge grad
    ...
    cuda_stream->Gather_Msg_To_Src_Mirror(
        f_input_grad_buffer, f_output_grad_buffer, device_csc->row_indices,
        device_csc->column_offset, device_csc->mirror_index, device_csc->v_size,
        feature_size);
    cuda_stream->CUDA_DEVICE_SYNCHRONIZE();
    return f_input_grad;
...
public:
  ...
  NtsVar forward(NtsVar &f_input) { // input edge  output vertex
    ...
    ValueType *f_output_buffer =
        graph_->Nts->getWritableBuffer(f_output, torch::DeviceType::CUDA);
    // LOG_INFO("owned_mirrors (%d)",partitioned_graph_->owned_mirrors);
    cuda_stream->Scatter_Dst_to_Msg(
        f_output_buffer, f_input_buffer, device_csc->row_indices,
...
public:
  ...
  NtsVar forward(NtsVar &f_input) { // input edge  output vertex
    ...
    NtsVar f_output = graph_->Nts->NewKeyTensor(
        {partitioned_graph_->owned_edges, feature_size},
        torch::DeviceType::CUDA);
    IntermediateResult = graph_->Nts->NewKeyTensor(
        {partitioned_graph_->owned_edges, feature_size},
        torch::DeviceType::CUDA);
    ValueType *f_input_buffer =
        graph_->Nts->getWritableBuffer(f_input, torch::DeviceType::CUDA);
    ValueType *f_output_buffer =
        graph_->Nts->getWritableBuffer(f_output, torch::DeviceType::CUDA);
    ValueType *f_cache_buffer = graph_->Nts->getWritableBuffer(
        IntermediateResult, torch::DeviceType::CUDA);
    // LOG_INFO("owned_mirrors (%d)",partitioned_graph_->owned_mirrors);
    cuda_stream->Edge_Softmax_Forward_Block(
        f_output_buffer, f_input_buffer, f_cache_buffer,
  ...
  NtsVar backward(NtsVar &f_output_grad) { // input vtx grad; output edge grad
    ...
    ValueType *f_cache_buffer = graph_->Nts->getWritableBuffer(
        IntermediateResult, torch::DeviceType::CUDA);
    cuda_stream->Edge_Softmax_Backward_Block(
        f_input_grad_buffer, f_output_grad_buffer, f_cache_buffer,
        device_csc->row_indices, device_csc->column_offset, device_csc->v_size,
        feature_size);
    cuda_stream->CUDA_DEVICE_SYNCHRONIZE();
    return f_input_grad;
