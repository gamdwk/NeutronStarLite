...
public:
  ...
  void GenerateWholeGraphTopo() {
    ...
    memcpy(column_offset, tmp_column_offset,
           sizeof(VertexId) * (owned_vertices + 1));
    memcpy(compressed_row_offset, tmp_row_offset,
           sizeof(VertexId) * (owned_mirrors + 1));
    for (VertexId i = 0; i < this->owned_edges; i++) {
  ...
  void DetermineMirror_() {
    ...
    graph_->process_edges_backward<int, VertexId>( // For EACH Vertex Processing
        ...
        [&](VertexId master, VertexId *msg) {
          // vertex master in local partition will have contribution to
          // partition part
          VertexId part = *msg;
    ...
    std::thread send_thread([&]() {
      for (int step = 1; step < partitions; step++) {
        ...
        MPI_Send(graph_chunks[recipient_id]->source_active->data,
                 WORD_OFFSET(graph_chunks[recipient_id]->source_active->size) +
                     1,
    ...
    std::thread recv_thread([&]() {
      for (int step = 1; step < partitions; step++) {
        ...
        MPI_Recv(this->hasMirrorAtPartition[sender_id]->data,
                 WORD_OFFSET(this->owned_vertices) + 1, MPI_UNSIGNED_LONG,
...
#pragma omp parallel for schedule(static, basic_chunk)
  ...
  void generatePartitionedSubgraph() {
    ...
    for (int i = 0; i < graph_->sockets; i++) {
      owned_edges +=
          (VertexId)(graph_->outgoing_adj_index[i])[graph_->vertices];
    }
  ...
  void
  PartitionToChunks(std::function<ValueType(VertexId, VertexId)> weight_compute,
                    DeviceLocation dt_) {
    graph_chunks.clear();
    ...
    for (VertexId i = 0; i < partitions; i++) {
      ...
      graph_chunks[i]->init(partition_offset[i], partition_offset[i + 1],
                            partition_offset[partition_id],
                            partition_offset[partition_id + 1], edgenumber[i],
                            dt_);
    ...
    for (VertexId i = 0; i < partitions; i++) {
      ...
      for (VertexId j = 0; j < graph_chunks[i]->edge_size; j++) {
        ...
        // vertexID so we can minus the start index to get the offset v_src_m
        // and v_dst_m is the real vertex id v_dst and v_src is local vertex id
      ...
      for (VertexId j = 0; j < graph_chunks[i]->batch_size_forward; j++) {
        ...
        for (VertexId e_idx = graph_chunks[i]->column_offset[j];
             e_idx < graph_chunks[i]->column_offset[j + 1]; e_idx++) {
...
#pragma omp parallel for
  ...
  void DistSchedulingMirror(
    ...
    size_t basic_chunk = 64;
    {
      // since we have only one partition. i think this loop should be discarded
#pragma omp parallel for
      for (VertexId begin_v_i = 0; begin_v_i < global_vertices;
           begin_v_i += basic_chunk) {
        // for every vertex, apply the sparse_slot at the partition
        // corresponding to the step
        VertexId v_i = begin_v_i;
  ...
  void TestGeneratedBitmap() {
    for (int i = 0; i < graph_chunks.size(); i++) {
      ...
      for (int j = graph_chunks[i]->dst_range[0];
           j < graph_chunks[i]->dst_range[1]; j++) {
        if (graph_chunks[i]->dst_get_active(j)) {
          count_act_dst++;
        }
      }
      for (int j = graph_chunks[i]->src_range[0];
           j < graph_chunks[i]->src_range[1]; j++) {
