...
public:
  ...
  NtsVar forward(NtsVar &f_input) {
    // f_input = input;
    NtsVar f_output =
        graph_->Nts->NewKeyTensor(f_input, torch::DeviceType::CPU);
  ...
  NtsVar backward(NtsVar &f_output_grad) {
    NtsVar f_input_grad =
        graph_->Nts->NewLeafTensor(f_output_grad, torch::DeviceType::CPU);
    ...
    memset(input_grad_buffer, 0,
           sizeof(ValueType) * f_output_grad.size(0) * f_output_grad.size(1));
