...
public:
  SampledSubgraph() { threads = std::max(numa_num_configured_cpus() - 1, 1); }
  ...
  void sample_load_destination(
      std::function<void(std::vector<VertexId> &destination)> dst_select,
      VertexId layer) {
    dst_select(sampled_sgs[layer]->dst()); // init destination;
  }

  void init_co(std::function<VertexId(VertexId dst)> get_nbr_size,
               VertexId layer) {
    if (layer == 0) {
  ...
  void sample_processing(
      std::function<void(VertexId fanout_i, VertexId dst,
...
#pragma omp parallel for
  ...
  void sample_postprocessing() {
    ...
    curr_dst_size =
        sampled_sgs[sampled_sgs.size() - 1]->get_distinct_src_size();
    curr_layer++;
  }

  void
  compute_one_layer(std::function<void(VertexId local_dst,
                                       std::vector<VertexId> &column_offset,
                                       //   std::vector<VertexId>&row_indices)>sparse_slot,VertexId
                                       //   layer,VertexId threads){
                    ...
                    VertexId layer) {
    {
      omp_set_num_threads(threads);
#pragma omp parallel for
      for (VertexId begin_v_i = 0; begin_v_i < sampled_sgs[layer]->dst().size();
           begin_v_i += 1) {
