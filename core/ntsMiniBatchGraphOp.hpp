...
NtsVar get_label(std::vector<VertexId> &dst, NtsVar &whole,
                 Graph<Empty> *graph) {
...
NtsVar get_feature(std::vector<VertexId> &src, NtsVar &whole,
                   Graph<Empty> *graph) {
  int feature_size = whole.size(1);
  NtsVar f_output = graph->Nts->NewKeyTensor({src.size(), feature_size},
...
public:
  ...
  NtsVar forward(NtsVar &f_input) {
    ...
    this->subgraphs->compute_one_layer(
        [&](VertexId local_dst, std::vector<VertexId> &column_offset,
          ...
          for (VertexId src_offset = src_start; src_offset < src_end;
               src_offset++) {
  ...
  NtsVar backward(NtsVar &f_output_grad) {
    ...
    NtsVar f_input_grad = graph_->Nts->NewLeafTensor(
        {subgraphs->sampled_sgs[layer]->src().size(), feature_size},
        torch::DeviceType::CPU);
    ...
    this->subgraphs->compute_one_layer(
        [&](VertexId local_dst, std::vector<VertexId> &column_offset,
          ...
          ValueType *local_input =
              f_output_grad_buffer + local_dst * feature_size;
          for (VertexId src_offset = src_start; src_offset < src_end;
               src_offset++) {
            VertexId local_src = subgraphs->sampled_sgs[layer]->r_i(src_offset);
            VertexId src = subgraphs->sampled_sgs[layer]->src()[local_src];
            ValueType *local_output =
                f_input_grad_buffer + local_src * feature_size;
            nts_acc(local_output, local_input,
                    nts_norm_degree(graph_, src, dst), feature_size);
          }
        },
        layer
...
//           nts_copy(f_output_buffer, eid, f_input_buffer, src,
//           feature_size,1);
//         }
//       },
...
//   VertexId* record;
//
//   SingleCPUDstAggregateOpMin(PartitionedGraph *partitioned_graph,VertexSubset
//   *active)
//       : ntsGraphOp(partitioned_graph, active) {
//     subgraphs = partitioned_graph->graph_chunks;
//   }
...
//  VertexId* record;
//
//  SingleCPUDstAggregateOpMax(PartitionedGraph *partitioned_graph,VertexSubset
//  *active)
//      : ntsGraphOp(partitioned_graph, active) {
//    subgraphs = partitioned_graph->graph_chunks;
//  }
