...
public:
  ...
  SingleCPUSrcDstScatterOp(PartitionedGraph *partitioned_graph,
                           VertexSubset *active)
  ...
  NtsVar forward(NtsVar &f_input) {
    ...
    NtsVar f_output = graph_->Nts->NewKeyTensor(
        {graph_->gnnctx->l_e_num, 2 * feature_size}, torch::DeviceType::CPU);
        ...
        [&](VertexId vtx, CSC_segment_pinned *subgraph, VertexId recv_id) {
          ...
          for (long eid = subgraph->column_offset[vtx];
            ...
            nts_copy(f_output_buffer, eid * 2, f_input_buffer, src,
                     feature_size, 1);
            nts_copy(f_output_buffer, eid * 2 + 1, f_input_buffer, vtx,
                     feature_size, 1);
          }
        },
  ...
  NtsVar backward(NtsVar &f_output_grad) {
    ...
    NtsVar f_input_grad = graph_->Nts->NewLeafTensor(
        {graph_->gnnctx->l_v_num, feature_size / 2}, torch::DeviceType::CPU);
...
public:
  ...
  SingleCPUSrcScatterOp(PartitionedGraph *partitioned_graph,
                        VertexSubset *active)
      : ntsGraphOp(partitioned_graph, active) {
    subgraphs = partitioned_graph->graph_chunks;
  }
  NtsVar forward(NtsVar &f_input) {
    int feature_size = f_input.size(1);
    NtsVar f_output = graph_->Nts->NewKeyTensor(
        {graph_->gnnctx->l_e_num, feature_size}, torch::DeviceType::CPU);
    ValueType *f_input_buffer =
        graph_->Nts->getWritableBuffer(f_input, torch::DeviceType::CPU);
    ValueType *f_output_buffer =
        graph_->Nts->getWritableBuffer(f_output, torch::DeviceType::CPU);
...
public:
  ...
  SingleCPUDstAggregateOp(PartitionedGraph *partitioned_graph,
                          VertexSubset *active)
      : ntsGraphOp(partitioned_graph, active) {
    subgraphs = partitioned_graph->graph_chunks;
  }
        ...
        [&](VertexId vtx, CSC_segment_pinned *subgraph, VertexId recv_id) {
          ...
          for (long eid = subgraph->column_offset[vtx];
            ...
            nts_acc(f_input_grad_buffer + (feature_size * eid),
                    f_output_grad_buffer + vtx * feature_size, feature_size);
          }
        },
        subgraphs, feature_size, active_);
    return f_input_grad;
...
public:
  ...
  SingleCPUDstAggregateOpMin(PartitionedGraph *partitioned_graph,
                             VertexSubset *active)
      : ntsGraphOp(partitioned_graph, active) {
    subgraphs = partitioned_graph->graph_chunks;
  }
    ...
    graph_->local_vertex_operation<int, ValueType>( // For EACH Vertex
        [&](VertexId vtx, CSC_segment_pinned *subgraph, VertexId recv_id) {
          ...
          nts_assign(f_input_grad_buffer,
                     f_output_grad_buffer + feature_size * vtx,
...
public:
  ...
  SingleCPUDstAggregateOpMax(PartitionedGraph *partitioned_graph,
                             VertexSubset *active)
      : ntsGraphOp(partitioned_graph, active) {
    subgraphs = partitioned_graph->graph_chunks;
  }
