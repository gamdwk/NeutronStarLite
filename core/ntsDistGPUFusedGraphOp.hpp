...
public:
  ...
  NtsVar forward(NtsVar &f_input) {
    ...
    NtsVar f_output =
        graph_->Nts->NewKeyTensor(f_input, torch::DeviceType::CUDA);
    ValueType *f_input_cpu_buffered =
        f_input_cpu.accessor<ValueType, 2>().data();

    { // original communication
      graph_->sync_compute_decoupled<int, ValueType>(
          f_input, subgraphs,
          [&](VertexId src) {
            graph_->NtsComm->emit_buffer(src,
                                         f_input_cpu_buffered +
                                             (src - graph_->gnnctx->p_v_s) *
                                                 feature_size,
                                         feature_size);
          },
  ...
  NtsVar backward(NtsVar &f_output_grad) {
    ...
    NtsVar f_input_grad =
        graph_->Nts->NewKeyTensor(f_output_grad, torch::DeviceType::CUDA);
    // if (!selective)
    {
