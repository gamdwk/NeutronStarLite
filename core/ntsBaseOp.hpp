...
inline void nts_comp_non_avx256(ValueType *output, ValueType *input,
                                ValueType weight, int feat_size) {
...
inline void nts_comp(ValueType *output, ValueType *input, ValueType weight,
  ...
  for (int i = 0; i < loop; i++) {
    __m256 source =
        _mm256_loadu_ps(reinterpret_cast<float const *>(&(input[i * LEN])));
    __m256 destination =
        _mm256_loadu_ps(reinterpret_cast<float const *>(&(output[i * LEN])));
    _mm256_storeu_ps(&(output[i * LEN]),
                     _mm256_add_ps(_mm256_mul_ps(source, w), destination));
  }
...
inline void nts_acc(ValueType *output, ValueType *input, ValueType weight,
                    int feat_size) {
  for (int i = 0; i < feat_size; i++) {
    // atomic add
 ...
 * @param feat_size feature size
 */
inline void nts_min(ValueType *output, ValueType *input, VertexId *record,
                    int feat_size, VertexId e_index) {
  for (int i = 0; i < feat_size; i++) {
    // atomic add
 ...
 * @param feat_size feature size
 */
inline void nts_max(ValueType *output, ValueType *input, VertexId *record,
                    int feat_size, VertexId e_index) {
  for (int i = 0; i < feat_size; i++) {
    // atomic add
 ...
 */
inline void nts_copy(ValueType *b_dst, long d_offset, ValueType *b_src,
                     VertexId s_offset, int feat_size, int counts) {
 ...
 * @return ValueType
 */
inline ValueType nts_norm_degree(Graph<Empty> *graph_, VertexId src,
                                 VertexId dst) {
