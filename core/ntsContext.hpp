...
public:
  ...
  NtsVar runGraphOp(PartitionedGraph *partitioned_graph, VertexSubset *active,
    ...
    // std::cout << "GraphOp "<<typeid(GOPT).name()<< " new" <<std::endl;
    nts::op::ntsGraphOp *curr = new GOPT(partitioned_graph, active);
    // std::cout << "GraphOp "<<typeid(GOPT).name()<< " forward" <<std::endl;
    NtsVar f_output = curr->forward(f_input);
    // std::cout << "GraphOp "<<typeid(GOPT).name()<< " forward end"
    // <<std::endl;
    if (this->training == true) {
      ...
      iot_id.push_back(
          IOTensorId((long)(f_output.data_ptr()), (long)(f_input.data_ptr())));
  ...
  NtsVar runGraphOp(PartitionedGraph *partitioned_graph, VertexSubset *active,
    ...
    iot_id.push_back(IOTensorId((long)(f_output.data_ptr()),
                                (long)(f_input1.data_ptr()),
                                (long)(f_input2.data_ptr())));
    // pre-alloc space to save graident
    output_grad.push_back(ig);
    count++;
    return f_output;
  ...
  NtsVar runSelfNNOp(std::function<NtsVar(NtsVar &, int)> vertexforward,
                     NtsVar &f_input, int layer_) { // graph op

    static_assert(std::is_base_of<nts::op::ntsNNBaseOp, NOPT>::value,
                  "template must be a type of graph op!");

    nts::op::ntsNNBaseOp *curr =
        new NOPT([&](NtsVar v_tensor,
                     int layer_) { return vertexforward(v_tensor, layer_); },
                 layer_);
    NtsVar f_output = curr->forward(f_input);
    if (this->training == true) {
      NtsVar ig;
  ...
  template <typename GOPT>
  NtsVar runGraphOp(SampledSubgraph *subgraphs_, Graph<Empty> *graph_,
                    int layer_,
                    NtsVar &f_input) { // graph op
  ...
  NtsVar
  runVertexForward(std::function<NtsVar(NtsVar &, NtsVar &)> vertexforward,
  ...
  void appendNNOp(NtsVar &input_t, NtsVar &output_t) {
    ...
    // we will chain the NNOP together, because torch lib will handle the
    // backward propagation when there is no graph operation
    if ((count > 0 && op.top() == NNOP) &&
        ((long)input_t.data_ptr()) == iot_id[iot_id.size() - 1].o_id) {
      output.pop();
      ...
      iot_id.push_back(
          IOTensorId((long)(output_t.data_ptr()), (long)(input_t.data_ptr())));
      // pre-alloc space to save graident
      output_grad.push_back(ig);
      count++;
    }
  ...
  void self_backward(bool retain_graph = true) {
    ...
    while (count > 1 || (count == 1 && NNOP == op.top())) {
      ...
      if (GRAPHOP == op.top()) {
        ...
        output_grad[preop_id] =
            ntsOp.top().get_graphop()->backward(output_grad[top_idx()]);
        //   LOG_INFO("input id %ld %d
        //   %d",preop_id,top_idx(),output_grad[preop_id].dim());
        // stable
        ...
        output_grad[preop_id] =
            ntsOp.top().get_graphop()->get_additional_grad();
        ...
        output_grad[preop_id] =
            ntsOp.top().get_nnop()->backward(output_grad[top_idx()]);
        // stable
        //       output_grad[top_idx()-1]=ntsOp.top().op->backward(output_grad[top_idx()]);
        pop_one_op();
  ...
  void debug() {
    ...
    while (!input.empty()) {
      if (i == 0) {
        LOG_INFO("input dim %d %d\t output dim %d \t OP type %d",
                 input.top().size(0), input.top().size(1), output.top().dim(),
                 op.top());
      } else {
        LOG_INFO("input dim %d %d \t output dim %d %d\t OP type %d",
                 input.top().size(0), input.top().size(1), output.top().size(0),
                 output.top().size(1), op.top());
      }
      input.pop();
      output.pop();
      op.pop();
