...
//  ValueType *f_input_cpu_buffered = f_input_cpu.accessor<ValueType,
//  2>().data();
//
...
//              src, f_input_cpu_buffered + (src - graph_->gnnctx->p_v_s) *
...
//      NtsVar
//      f_input_grad=graph_->Nts->NewKeyTensor(f_output_grad,torch::DeviceType::CUDA);
//  // if (!selective)
//  {
...
//  std::vector<CSC_segment_pinned *> subgraphs;
//
//  ForwardSingleGPUfuseOp(PartitionedGraph *partitioned_graph,VertexSubset
//  *active)
//      : ntsGraphOp(partitioned_graph, active) {
//    subgraphs = partitioned_graph->graph_chunks;
//  }
...
//    int feature_size = f_input.size(1);
//    NtsVar
//    f_output=graph_->Nts->NewKeyTensor(f_input,torch::DeviceType::CUDA);
//    graph_->forward_single<int, ValueType>(f_input, subgraphs, f_output,
//    feature_size); return f_output;
//  }
//
//  NtsVar backward(NtsVar &f_output_grad){
//    int feature_size = f_output_grad.size(1);
...
//  std::vector<CSC_segment_pinned *> subgraphs;
//
//  SingleCPUSrcDstScatterOp(PartitionedGraph *partitioned_graph,VertexSubset
//  *active)
//      : ntsGraphOp(partitioned_graph, active) {
//    subgraphs = partitioned_graph->graph_chunks;
//  }
...
//          nts_copy(f_output_buffer, eid * 2, f_input_buffer, src,
//          feature_size,1); nts_copy(f_output_buffer, eid * 2 + 1,
//          f_input_buffer, vtx, feature_size,1);
//        }
//      },
...
//                    f_output_grad_buffer + (feature_size * eid * 2),
//                    feature_size);
...
//  std::vector<CSC_segment_pinned *> subgraphs;
//
//  SingleCPUSrcScatterOp(PartitionedGraph *partitioned_graph,VertexSubset
//  *active)
//      : ntsGraphOp(partitioned_graph, active) {
//    subgraphs = partitioned_graph->graph_chunks;
//  }
...
//  std::vector<CSC_segment_pinned *> subgraphs;
//
//  SingleCPUDstAggregateOp(PartitionedGraph *partitioned_graph,VertexSubset
//  *active)
//      : ntsGraphOp(partitioned_graph, active) {
//    subgraphs = partitioned_graph->graph_chunks;
//  }
...
//  NtsVar forward(NtsVar &f_input) {
//    //f_input = input;
//    NtsVar f_output = graph_->Nts->NewKeyTensor(f_input,
//    torch::DeviceType::CPU); ValueType *f_input_buffer =
//        graph_->Nts->getWritableBuffer(f_input, torch::DeviceType::CPU);
//    ValueType *f_output_buffer =
//        graph_->Nts->getWritableBuffer(f_output, torch::DeviceType::CPU);
...
//                  src,
//                  f_input_buffer + (src - graph_->gnnctx->p_v_s) *
//                  feature_size, write_index, feature_size);
//            }
//          } else {
...
//        // recv_id represent the partition ID corresponding to current
//        subgraph
...
//          // for every vertex, accumulate the incoming feature though
//          iterating
//          // column vertices
...
//            nts_comp(local_output, local_input, nts_norm_degree(graph_, src,
//            dst),
//                 feature_size);
//          }
//        },
...
//  }
//  NtsVar backward(NtsVar &f_output_grad) {
//    NtsVar f_input_grad=graph_->Nts->NewLeafTensor(f_output_grad,
//    torch::DeviceType::CPU);
//  ValueType *output_grad_buffer =
//      graph_->Nts->getWritableBuffer(f_output_grad, torch::DeviceType::CPU);
//  ValueType *input_grad_buffer =
//      graph_->Nts->getWritableBuffer(f_input_grad, torch::DeviceType::CPU);
//  memset(input_grad_buffer, 0, sizeof(ValueType) * f_output_grad.size(0) *
//  f_output_grad.size(1));
...
//          nts_comp(local_output_buffer, local_input_buffer,
//          nts_norm_degree(graph_,src, dst),
//               feature_size);
//        }
//        if (graph_->rtminfo->lock_free) {
...
//        nts_acc(input_grad_buffer + (src - graph_->gnnctx->p_v_s) *
//        feature_size, msg, feature_size); return 1;
//      },
...
//    NtsVar
//    f_output=graph_->Nts->NewKeyTensor({partitioned_graph_->owned_mirrors,
//                feature_size},torch::DeviceType::CPU);
//    ValueType *f_input_buffer =
//      graph_->Nts->getWritableBuffer(f_input, torch::DeviceType::CPU);
//    ValueType *f_output_buffer =
//      graph_->Nts->getWritableBuffer(f_output, torch::DeviceType::CPU);
//    //LOG_INFO("owned_mirrors (%d)",partitioned_graph_->owned_mirrors);
//      graph_->get_from_dep_neighbor_mutisockets<int, ValueType>( // For EACH
//      Vertex Processing
//      [&](VertexId src, int current_send_partition) {
//        if (graph_->rtminfo->lock_free) {
//            VertexId src_trans = src - graph_->gnnctx->p_v_s;
...
//                * feature_size, recv_buffer, sizeof(ValueType) *
//                feature_size);
//        return 0;
//      },
//      subgraphs, feature_size, active_);
...
//            f_output_grad_buffer + partitioned_graph_->MirrorIndex[src] *
//            feature_size;
//        if (graph_->rtminfo->lock_free) {
//          if (subgraphs[recv_id]->src_get_active(src)) {
//            VertexId write_index =
...
//    //LOG_INFO("owned_mirrors (%d)",partitioned_graph_->owned_mirrors);
//    NtsVar
//    f_output=graph_->Nts->NewKeyTensor({partitioned_graph_->owned_edges,
//                feature_size},torch::DeviceType::CPU);
//    ValueType *f_input_buffer =
//      graph_->Nts->getWritableBuffer(f_input, torch::DeviceType::CPU);
//    ValueType *f_output_buffer =
...
//    int feature_size=f_output_grad.size(1);
//    NtsVar
//    f_input_grad=graph_->Nts->NewLeafTensor({partitioned_graph_->owned_mirrors,
//                feature_size},torch::DeviceType::CPU);
//    ValueType *f_input_grad_buffer =
//      graph_->Nts->getWritableBuffer(f_input_grad, torch::DeviceType::CPU);
//    ValueType *f_output_grad_buffer =
...
//    //LOG_INFO("owned_mirrors (%d)",partitioned_graph_->owned_mirrors);
//    NtsVar
//    f_output=graph_->Nts->NewKeyTensor({partitioned_graph_->owned_vertices,
//                feature_size},torch::DeviceType::CPU);
//    ValueType *f_input_buffer =
//      graph_->Nts->getWritableBuffer(f_input, torch::DeviceType::CPU);
//    ValueType *f_output_buffer =
...
//    int feature_size=f_output_grad.size(1);
//    NtsVar
//    f_input_grad=graph_->Nts->NewLeafTensor({partitioned_graph_->owned_edges,
//                feature_size},torch::DeviceType::CPU);
//    ValueType *f_input_grad_buffer =
//      graph_->Nts->getWritableBuffer(f_input_grad, torch::DeviceType::CPU);
//    ValueType *f_output_grad_buffer =
