...
class Sampler {
public:
  std::vector<SampledSubgraph *>
      work_queue; // excepted to be single write multi read
  ...
  void reservoir_sample(int layers_, int batch_size_,
                        std::vector<int> fanout_) {
    assert(work_offset < work_range[1]);
    int actual_batch_size =
        std::min((VertexId)batch_size_, work_range[1] - work_offset);
    ...
    for (int i = 0; i < layers_; i++) {
      ...
      ssg->init_co(
          ...
          i);
      ssg->sample_processing([&](VertexId fanout_i, VertexId dst,
